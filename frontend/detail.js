...
// Ambil slug dari path /video/SLUG
function getSlug() {
  ...
  return `
    <a href="/video/${v.slug}" class="group block">
      <div class="relative aspect-video rounded-lg overflow-hidden border border-pink-800/50 group-hover:border-pink-600/60 transition-colors" style="background:#3b001a">
        ${thumb ? `<img src="${thumb}" alt="${title}" class="h-full w-full object-cover" loading="lazy" onerror="this.style.display='none'">` : ''}
  ...
  return `
    <a href="/video/${v.slug}" class="flex gap-2 sm:gap-3 group rounded-xl hover:bg-pink-900/40 p-2 -mx-2 transition-colors">
      <div class="relative w-[120px] sm:w-[140px] shrink-0 aspect-video rounded-lg overflow-hidden bg-pink-900 border border-pink-800/50">
        ${thumb ? `<img src="${thumb}" alt="${title}" class="w-full h-full object-cover" loading="lazy" onerror="this.style.display='none'">` : ''}
      </div>
...
async function loadDetail() {
  ...
  try {
    const resp = await fetchWithTimeout(`${API}/video/${encodeURIComponent(slug)}`, 20000);
...
function renderVideo(data) {
  ...
  if (postedEl) {
    const span = postedEl.querySelector('span');
    if (span) span.textContent = data.postedAgo || '';
    postedEl.classList.toggle('hidden', !data.postedAgo);
  }

  // Views & likes (tidak tersedia dari bokepcolmek.me listing, sembunyikan)
  ...
  // Player setup — embed URL sudah tersedia langsung dari data.embedUrl
  const overlay = document.getElementById('playerOverlay');
  const frame   = document.getElementById('playerFrame');
  const noEmbed = document.getElementById('noEmbed');
  const playBtn = document.getElementById('playBtn');

  // Ambil embed URL: prioritaskan dari data.embedUrl, fallback ke /embed endpoint
  let embedPromise;
  if (data.embedUrl) {
    embedPromise = Promise.resolve({ embedUrl: data.embedUrl, type: 'iframe' });
  } else {
    embedPromise = fetchWithTimeout(`${API}/video/${encodeURIComponent(data.slug)}/embed`, 15000)
      .then(r => r.json())
  ...
  function showEmbed(url, type) {
    ...
    if (type === 'direct' || isM3U8 || isMP4) {
      // Direct stream — play dengan HLS.js atau native <video>
      ...
      // Iframe embed (xvideos, xhamster, dll via fbplay.vip atau langsung)
      // Tidak perlu sandbox ketat — embed dari third-party player sah, bukan indoav
