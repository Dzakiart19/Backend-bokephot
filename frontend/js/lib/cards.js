...
// Directlink ad URL — buka di tab baru setiap klik video
const AD_URL = 'https://rm358.com/4/11476496';
const AD_ONCLICK = `window.open('${AD_URL}','_blank','noreferrer,noopener')`;
...
export function buildCard(v) {
  ...
  const dur   = v.duration
    ...
    : '';
  return `
    <a href="/video/${v.slug}" class="group block hover-card" onclick="${AD_ONCLICK}">
...
export function buildRelatedCardList(v) {
  ...
  const title = escHtml(v.title);
  return `
    <a href="/video/${v.slug}" class="flex gap-2 sm:gap-3 group rounded-xl hover:bg-pink-900/40 p-2 -mx-2 transition-colors" onclick="${AD_ONCLICK}">
...
export function buildRelatedCardGrid(v) {
  ...
  const title = escHtml(v.title);
  return `
    <a href="/video/${v.slug}" class="group block" onclick="${AD_ONCLICK}">
