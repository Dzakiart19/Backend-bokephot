...
export function buildCard(v) {
  ...
  return `
    <a href="/video/${v.slug}" class="group block hover-card" data-ad-click="1">
      <div class="relative aspect-video rounded-lg sm:rounded-xl overflow-hidden border border-zinc-700/50 group-hover:border-red-600/60 transition-colors" style="background:#2a2a2a">
        ...
        <div class="absolute inset-0 bg-gradient-to-t from-[#1e1e1e]/70 via-transparent to-transparent"></div>
        ${dur}
        <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          <div class="w-10 h-10 rounded-full bg-red-500/80 flex items-center justify-center">
            <svg class="w-5 h-5 text-white ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
          </div>
        </div>
      </div>
      <div class="mt-1.5 sm:mt-2 px-0.5">
        <h3 class="text-[11px] sm:text-[13px] font-semibold text-zinc-50 leading-snug line-clamp-2 group-hover:text-zinc-300 transition-colors">${title}</h3>
      </div>
    </a>`;
}
...
export function buildRelatedCardList(v) {
  ...
  const title = escHtml(v.title);
  return `
    <a href="/video/${v.slug}" class="flex gap-2 sm:gap-3 group rounded-xl hover:bg-zinc-800/40 p-2 -mx-2 transition-colors" data-ad-click="1">
      <div class="relative w-[120px] sm:w-[140px] shrink-0 aspect-video rounded-lg overflow-hidden bg-zinc-800 border border-zinc-700/50">
      ...
      <div class="flex-1 min-w-0 py-0.5">
        <h4 class="text-[12px] font-semibold text-zinc-100 group-hover:text-white line-clamp-2 leading-snug">${title}</h4>
      </div>
    </a>`;
}
...
export function buildRelatedCardGrid(v) {
  ...
  return `
    <a href="/video/${v.slug}" class="group block" data-ad-click="1">
      <div class="relative aspect-video rounded-lg overflow-hidden border border-zinc-700/50 group-hover:border-red-600/60 transition-colors" style="background:#2a2a2a">
        ${thumb ? `<img src="${escAttr(thumb)}" alt="${title}" class="h-full w-full object-cover" loading="lazy" onerror="this.style.display='none'">` : ''}
        <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          <div class="w-8 h-8 rounded-full bg-red-500/80 flex items-center justify-center">
            <svg class="w-4 h-4 text-white ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
          </div>
        </div>
      </div>
      <div class="mt-1.5">
        <h4 class="text-[11px] font-semibold text-zinc-100 line-clamp-2 leading-snug group-hover:text-white">${title}</h4>
      </div>
    </a>`;
}
